...
function Start-Backend {
    ...
    $requiredPackages = @("express", "cors", "multer", "googleapis")
    
    foreach ($package in $requiredPackages) {
        $packagePath = Join-Path $ProjectRoot "node_modules\$package"
        if (-not (Test-Path $packagePath)) {
            Write-Log "Missing package: $package" -Color "Red"
            $dependencyCheck = $false
        }
    }
    
    if (-not $dependencyCheck) {
        Write-Log "Installing missing dependencies..." -Color "Yellow"
...
switch ($Action.ToLower()) {
    ...
    "status" {
        ...
        try {
            $frontendProcesses = Get-NetTCPConnection -LocalPort 8000 -ErrorAction SilentlyContinue
