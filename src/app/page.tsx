...
import { getValasByNumber, getValasByCode, getMainCurrencyCode } from "@/lib/valasData"
import type { KursData } from "@/utils/currencyValidation"
// Helper untuk normalisasi kode currency sesuai kurs-mbarate
function normalizeCurrencyCode(input: string, kursData: KursData[]): string | null {
  if (!input) return null;
  const kodeInput = input.toUpperCase().trim();
  // USK/USB alias ke USD
  let mainCode = getMainCurrencyCode({ kode: kodeInput });
  if (mainCode === 'USD') {
    // Cari baris USD di kursData
    const usd = kursData.find(k => k.currency.toUpperCase().includes('USD'));
    if (usd) return usd.currency;
  }
  // Cek exact match di kursData
  const exact = kursData.find(k => k.currency.toUpperCase() === kodeInput);
  if (exact) return exact.currency;
  // Cek match dengan awalan kode + spasi/:
  const prefix = kursData.find(k => k.currency.toUpperCase().startsWith(kodeInput + ' ') || k.currency.toUpperCase().startsWith(kodeInput + ':'));
  if (prefix) return prefix.currency;
  // Cek alias khusus lain (EUR, CNY, KRW, TWD, QAR)
  if (kodeInput === 'EUR') {
    const eur = kursData.find(k => k.currency.toUpperCase().includes('EURO') || k.currency.toUpperCase().includes('EUR'));
    if (eur) return eur.currency;
  }
  if (kodeInput === 'CNY') {
    const cny = kursData.find(k => k.currency.toUpperCase().includes('YUAN'));
    if (cny) return cny.currency;
  }
  if (kodeInput === 'KRW') {
    const krw = kursData.find(k => k.currency.toUpperCase().includes('WON'));
    if (krw) return krw.currency;
  }
  if (kodeInput === 'TWD') {
    const twd = kursData.find(k => k.currency.toUpperCase().includes('NT'));
    if (twd) return twd.currency;
  }
  if (kodeInput === 'QAR') {
    const qar = kursData.find(k => k.currency.toUpperCase().includes('QTR'));
    if (qar) return qar.currency;
  }
  return null;
}
...
// Helper functions for AHK script generation
...
function truncateMaxLength(text: string, maxLength: number): string {
  return text.length > maxLength ? text.substring(0, maxLength) : text
}

// Function to generate and download AHK script menggunakan API
async function generateAndDownloadAHK(data: any, valasTransactions: any[] = [], pembayaranRp: string = '') {
  try {
    // Buat array transaksi dari valasTransactions yang aktif
    const transactions = valasTransactions.map(t => ({
      currency: t.valas,
      amount: t.jumlah,
      rate: t.rate
    }));
    
    // Ambil data pembayaran BNS dari parameter
    const jenisTx = data.transactionType || data.jenisTransaksi || 'BNB';
    let pembayaranData = null;
    
    // Jika BNS, ambil data pembayaran dari parameter
    if (jenisTx === 'BNS' && pembayaranRp) {
      pembayaranData = pembayaranRp.replace(/[^0-9]/g, ''); // Hapus formatting, ambil angka saja
      ...
      console.log('  - jenisTransaksi:', jenisTx)
      console.log('  - pembayaranRp:', pembayaranRp)
    }
    ...
    const requestData = {
      ...
      transactionType: jenisTx,
      // Kirim array transaksi
      transactions: transactions,
      // Fallback untuk backward compatibility
      currency: transactions[0]?.currency || 'USD',
      amount: transactions[0]?.amount || '100.00',
      rate: transactions[0]?.rate || '15750.00',
      // Data pembayaran BNS
    ...
    if (result.success) {
      console.log('✅ Skrip AHK berhasil dijalankan langsung!');
      console.log('📋 Details:', result);
      // Tampilkan notifikasi sukses jika ada toast
...
export default function Home() {
  ...
  // 13 rows: 1st row kosong, sisanya kosong (tanpa placeholder)
  const [valasRows, setValasRows] = useState([
    { kode: '', valas: '', valas2: '', jumlah: '', rate: '', hasil: '' },
    ...Array(12).fill({ kode: '', valas: '', valas2: '', jumlah: '', rate: '', hasil: '' })
  ]);
  ...
  const cetakButtonRef = useRef<HTMLButtonElement>(null);
  ...
  // Jumlah total rupiah - hanya dari baris yang terisi (bukan baris pertama dan bukan kosong)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Generate and download AHK script after successful save
      await generateAndDownloadAHK(formData, activeValasTransactions, pembayaranRp);
  ...
  const handleCekClick = () => {
    ...
    // Jika input angka (termasuk -1), ambil dari getValasByNumber (urutan sama dengan Daftar Valas)
    if (/^-?\d+$/.test(kodeInput)) {
      const nomor = parseInt(kodeInput);
      const valas = getValasByNumber(nomor);
      if (valas) {
        setValasRows(rows => rows.map((row, idx) => idx === 0 ? { ...row, valas: '', valas2: valas.kode } : row));
        setTimeout(() => { if (amountRef.current) amountRef.current.focus(); }, 100);
      } else {
        // Coba mapping alias USK/USB jika -1/1
        if (nomor === -1) {
          setValasRows(rows => rows.map((row, idx) => idx === 0 ? { ...row, valas: '', valas2: 'USK' } : row));
          setTimeout(() => { if (amountRef.current) amountRef.current.focus(); }, 100);
        } else if (nomor === 1) {
          setValasRows(rows => rows.map((row, idx) => idx === 0 ? { ...row, valas: '', valas2: 'USB' } : row));
          setTimeout(() => { if (amountRef.current) amountRef.current.focus(); }, 100);
        } else {
    ...
    // Jika input kode/alias, gunakan normalisasi kurs
    const normalized = normalizeCurrencyCode(kodeInput, kursData);
    if (normalized) {
      setValasRows(rows => rows.map((row, idx) => idx === 0 ? { ...row, valas: '', valas2: normalized } : row));
      setTimeout(() => { if (amountRef.current) amountRef.current.focus(); }, 100);
    } else {
      toast.error(`Kode mata uang ${kodeInput} tidak ditemukan di daftar kurs`);
    }
  };
          ...
          <div style={{ width: '50%', background: '#FFFFFF', minHeight: 400, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', padding: 12 }}>
            ...
            <table className="w-full border border-black rounded text-sm mb-2 table-fixed bg-white">
              ...
              <tbody>
                {valasRows.slice(1).map((row, idx) => (
                  <tr key={idx + 1}>
                    <td className="border border-black px-1 py-1 text-center font-bold" style={{width: '30px'}}>{idx + 1}</td>
            ...
            {jenisTransaksi === 'BNS' && (
              <>
                <div className="flex justify-end items-center mt-1">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                        // Setelah input pembayaran, fokus ke tombol Cetak
                        if (cetakButtonRef.current) {
                          cetakButtonRef.current.focus();
                        }
                      }
                    }}
                    min="0"
                ...
                {/* Tombol Cetak di bawah field Terbilang untuk transaksi BNS */}
                <div className="flex justify-end items-center mt-2">
                  <button
                    ref={cetakButtonRef}
                    type="button"
                    className="border border-black px-4 py-2 rounded font-semibold bg-purple-700 text-white hover:bg-purple-800 transition-colors"
                    ...
                    title="Simpan semua transaksi dan cetak"
                  >
                    Cetak
                  </button>
                </div>
            ...
            <button
              ...
              onClick={(e) => {
                if (jenisTransaksi === 'BNS') {
                  // Untuk BNS, fokus ke field pembayaran
                  if (pembayaranRef.current) {
                    pembayaranRef.current.focus();
                    pembayaranRef.current.select();
                  }
                } else {
                  // Untuk BNB, langsung submit
                  handleSubmit(e);
                }
              }}
              onKeyDown={(e) => {
              ...
              disabled={!isTransactionAreaOpen}
              title={!isTransactionAreaOpen ? "Tekan tombol Lanjut pada form nasabah terlebih dahulu" : jenisTransaksi === 'BNS' ? "Fokus ke field pembayaran" : "Simpan semua transaksi"}
            >
              Selesai {jenisTransaksi === 'BNS' ? 'Bayar' : 'Cetak'}
            </button>
            <button
