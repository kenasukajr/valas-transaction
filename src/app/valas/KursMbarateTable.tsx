...
export default function KursMbarateTable({ refreshTrigger, selectedCurrency }: KursMbarateTableProps) {
  ...
  if (selectedCurrency && selectedCurrency.trim() !== '') {
    // Normalisasi selectedCurrency ke kode utama (USD jika USK/USB)
    const selectedCurrencyMain = getMainCurrencyCode({ kode: selectedCurrency.trim().toUpperCase() })?.toUpperCase() || selectedCurrency.trim().toUpperCase();
    // Cari baris yang cocok dengan mata uang yang dipilih atau alias USD
    const matchedRows: KursRow[] = [];
    const otherRows: KursRow[] = [];
    kursSorted.forEach(row => {
      const kode = (row.kode || '').toUpperCase();
      // Untuk baris USD, USK, USB: highlight jika selectedCurrencyMain adalah USD
      const kodeMain = getMainCurrencyCode({ kode })?.toUpperCase() || kode;
      const isMatch =
        kodeMain === selectedCurrencyMain ||
        // Match dengan currency yang diikuti spasi atau ":"
        kode.startsWith(selectedCurrencyMain + ' ') ||
        kode.startsWith(selectedCurrencyMain + ':') ||
        // Match untuk USD variants (USD New, USD 2nd, dll)
        (selectedCurrencyMain === 'USD' && kode.includes('USD')) ||
        // Match untuk EURO
        (selectedCurrencyMain === 'EUR' && (kode.includes('EURO') || kode.includes('EUR'))) ||
        // Match untuk YUAN/CNY
        (selectedCurrencyMain === 'CNY' && kode.includes('YUAN')) ||
        // Match untuk WON/KRW
        (selectedCurrencyMain === 'KRW' && kode.includes('WON')) ||
        // Match untuk NT/TWD
        (selectedCurrencyMain === 'TWD' && kode.includes('NT')) ||
        // Match untuk QTR/QAR
        (selectedCurrencyMain === 'QAR' && kode.includes('QTR'));
      if (isMatch) {
            ...
            <tbody>
              {kursDisplay.length === 0 ? (
                ...
                const isSelectedCurrency = selectedCurrency && selectedCurrency.trim() !== '' ? (() => {
                  // Normalisasi selectedCurrency ke kode utama (USD jika USK/USB)
                  let selectedCurrencyMain = getMainCurrencyCode({ kode: selectedCurrency.trim().toUpperCase() })?.toUpperCase() || selectedCurrency.trim().toUpperCase();
                  // Jika selectedCurrency adalah USK/USB, treat as USD
                  if (selectedCurrencyMain === 'USK' || selectedCurrencyMain === 'USB') selectedCurrencyMain = 'USD';
                  const kode = (row.kode || '').toUpperCase();
                  let kodeMain = getMainCurrencyCode({ kode })?.toUpperCase() || kode;
                  if (kodeMain === 'USK' || kodeMain === 'USB') kodeMain = 'USD';
                  return (
                    kodeMain === selectedCurrencyMain ||
                    kode.startsWith(selectedCurrencyMain + ' ') ||
                    kode.startsWith(selectedCurrencyMain + ':') ||
