...
export const UserFormNasabah: React.FC<UserFormNasabahProps> = ({
  ...
  React.useEffect(() => {
    if (typeof formData === 'object' && formData) {
      if (!arraysEqual(formData.images, images)) {
        // Hanya sync jika berbeda dan tidak kosong
        if (images.length > 0 || (formData.images && formData.images.length > 0)) {
          if ('onImagesChange' in formData && typeof formData.onImagesChange === 'function') {
            formData.onImagesChange(images);
          } else if ('setFormData' in formData && typeof formData.setFormData === 'function') {
            // Gunakan async untuk mencegah blocking dan loop
            setTimeout(() => {
  ...
  const handleLocalImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Upload file ke server untuk mendapatkan URL file path
    ...
    try {
      ...
      // Tambahkan URL file path (bukan base64) ke array images
      setImages(prev => {
  ...
  const handleLocalClearImage = () => {
    setImages(prev => {
      ...
      // HANYA set state, biarkan useEffect([images]) yang handle sync ke parent
      return arr;
    });
  };
