  ...
  useEffect(() => {
    ...
    if (formData.images && Array.isArray(formData.images) && formData.images.length > 0) {
      ...
      if (imageSource === 'suggestion' && !previewSuggestion) {
        console.log(`  → Preview suggestion hilang, clear images (jangan fallback ke formData.images)`);
        setImages([]);
        setActiveImageIdx(0);
        setImageSource(null);
      ...
      // Jika imageSource bukan 'suggestion', ini adalah upload manual yang harus tetap tampil
      ...
      // PERBAIKAN: Sama seperti di atas, jangan fallback jika imageSource adalah 'suggestion'
      if (imageSource === 'suggestion' && !previewSuggestion) {
        console.log(`  → Preview suggestion hilang, clear images (jangan fallback ke formData.image)`);
        setImages([]);
        setActiveImageIdx(0);
        setImageSource(null);
        prevImagesLength.current = 0;
